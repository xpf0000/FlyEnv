      ...
      <el-table :data="alias" style="width: 100%" :border="true" :show-overflow-tooltip="true">
        <el-table-column width="120px" align="left" :label="I18nT('service.alias')">
          <template #default="scope">
            <el-button link @click.stop="copy()">{{ scope.row.name }}</el-button>
          </template>
        </el-table-column>
          ...
          <el-table-column width="120px" align="left" prop="name" :label="I18nT('host.phpVersion')">
            <template #default="scope">
              <template v-if="scope.row?.php?.bin">
                <el-button link @click.stop="open()">php-{{ scope.row.php.version }}</el-button>
              </template>
            </template>
          </el-table-column>
        </template>
...
<script lang="ts" setup>
  ...
  import { AsyncComponentSetup, AsyncComponentShow, join } from '@web/fn'
  ...
  const alias = computed(() => {
    ...
    return list.map((item) => {
      return {
        ...item,
        path: join('/Users/xxx/Library/PhpWebStudy/alias', `${item.name}`)
      }
    })
  })
