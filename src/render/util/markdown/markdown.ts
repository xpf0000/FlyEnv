...
export async function createMarkdownRenderer(): Promise<MarkdownRenderer> {
  ...
  const theme: ThemeOptions = { light: 'github-light', dark: 'github-dark' }
  const codeCopyButtonTitle = 'Copy Code'
  const hasSingleTheme = typeof theme === 'string' || 'name' in theme

  const [highlight, dispose] = await createHighlighter(theme, {}, console)

  _disposeHighlighter = dispose

  md = new MarkdownItAsync({ html: true, linkify: true, highlight, xhtmlOut: true })
  ...
  md.use(highlightLinePlugin)
    ...
    .use(imagePlugin)
    .use(linkPlugin, { target: '_blank', rel: 'noreferrer' }, '/')
    .use(lineNumberPlugin, false)
  ...
  } as anchorPlugin.AnchorOptions).use(frontmatterPlugin, {} as FrontmatterPluginOptions)
  ...
  md.use(sfcPlugin, {} as SfcPluginOptions)
    .use(titlePlugin)
    .use(tocPlugin, {} as TocPluginOptions)
