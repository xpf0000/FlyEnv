<template>
  <div class="soft-index-panel main-right-panel">
    <el-radio-group v-model="tab" class="mt-3">
      <template v-for="(item, _index) in tabs" :key="_index">
        <el-radio-button :label="item" :value="_index"></el-radio-button>
      </template>
    </el-radio-group>
    <div class="main-block">
      <Service v-if="tab === 0" type-flag="frankenphp" title="FrankenPHP">
        <template #column>
          <el-table-column label="PHP" prop="php" width="90px" align="center"> </el-table-column>
          <el-table-column label="Caddy" prop="caddy" width="90px" align="center">
          </el-table-column>
        </template>
        <template #action="{ row }">
          <VersionActions :item="row" />
        </template>
      </Service>
      <Manager
        v-else-if="tab === 1"
        type-flag="frankenphp"
        :has-static="true"
        :show-port-lib="false"
        :show-sdkman-lib="false"
        url="https://github.com/php/frankenphp/releases"
        title="FrankenPHP"
      ></Manager>
      <Config v-if="tab === 2"></Config>
      <Logs v-if="tab === 3"></Logs>
    </div>
  </div>
</template>

<script lang="ts" setup>
  import Service from '../ServiceManager/index.vue'
  import Config from './Config.vue'
  import Logs from './Logs.vue'
  import VersionActions from './VersionActions.vue'
  import Manager from '../VersionManager/index.vue'
  import { AppModuleSetup } from '@/core/Module'
  import { I18nT } from '@lang/index'

  const { tab, checkVersion } = AppModuleSetup('frankenphp')
  const tabs = [
    I18nT('base.service'),
    I18nT('base.versionManager'),
    I18nT('base.configFile'),
    I18nT('base.log')
  ]
  checkVersion()
</script>
