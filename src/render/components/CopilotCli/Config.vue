    ...
    <el-card class="app-base-el-card flex-1 overflow-hidden">
      <template #header>
        <div class="flex items-center justify-between">
          ...
          <el-button link :disabled="CopilotCliSetup.loading" @click="CopilotCliSetup.init()">
            <yb-icon
...
<script lang="ts" setup>
  ...
  import { CopilotCliSetup } from '@/components/CopilotCli/setup'

  const conf = ref()

  const configs = computed(() => {
    return Object.entries(CopilotCliSetup.configPaths).map(([name, path]) => ({ name, path }))
  })

  const filepath = computed({
    get() {
      return CopilotCliSetup.confTab
    },
    set(value) {
      CopilotCliSetup.confTab = value
    }
  })
