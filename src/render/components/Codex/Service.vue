<template>
  <el-card class="version-manager">
      ...
      <div class="card-header">
        <div class="left">
          ...
          <el-dropdown>
            ...
            <template #dropdown>
              <el-dropdown-item @click.stop="CodexSetup.openURL('home')">
                {{ I18nT('codex.officialWebsite') }}
              </el-dropdown-item>
              <el-dropdown-item @click.stop="CodexSetup.openURL('docs')">
                {{ I18nT('codex.officialDocs') }}
              </el-dropdown-item>
            </template>
          </el-dropdown>
        </div>
        <el-button
          class="button"
          :disabled="CodexSetup.loading || CodexSetup.installing"
          link
          @click="CodexSetup.init()"
        >
          <yb-icon
    ...
    <div class="w-full h-full overflow-hidden">
      ...
      <template v-else-if="!CodexSetup.loading && !CodexSetup.installed">
        <div class="p-5">
          <pre class="app-html-block" v-html="I18nT('codex.notInstalled')"></pre>
          ...
          <el-button
            type="primary"
            class="mt-5"
            :disabled="CodexSetup.installing"
            @click.stop="installCodex"
            >{{ I18nT('base.install') }}</el-button
          >
        </div>
      </template>
      <template v-else-if="CodexSetup.installed">
        <div class="p-5 h-full overflow-hidden">
          <el-form
            ...
            @submit.prevent
          >
            <el-form-item class="flex-shrink-0" :label="'Codex ' + I18nT('base.version')">
              <span
                class="whitespace-pre-wrap text-sm text-gray-600 dark:text-gray-300 leading-relaxed"
                >{{ CodexSetup.version }}</span
              >
            </el-form-item>
            <el-form-item
              ...
              <el-scrollbar class="w-full">
                <div class="w-full command-categories">
                  <el-collapse v-model="activeCategories">
                    <el-collapse-item
                      v-for="(category, cIndex) in CodexSetup.commandData.categories"
                      :key="cIndex"
      ...
      <template v-else-if="CodexSetup.loading">
