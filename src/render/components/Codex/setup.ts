...
class Codex {
  ...
  deleteSessions(sessionIds: string[]): Promise<string[]> {
    ...
    return new Promise((resolve) => {
      IPC.send('app-fork:codex', 'deleteSessions', ids).then((key: string, res: any) => {
        IPC.off(key)
  ...
  startSessionInTerminal(workDir: string): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      IPC.send('app-fork:codex', 'runInTerminal', workDir).then((key: string, res: any) => {
        IPC.off(key)
  ...
  resumeLastSessionInTerminal(workDir: string): Promise<boolean> {
    ...
    this.openingSessionDirs.add(workDir)
    return new Promise((resolve) => {
      IPC.send('app-fork:codex', 'resumeLastSessionInTerminal', workDir).then(
        (key: string, res: any) => {
          IPC.off(key)
          this.openingSessionDirs.delete(workDir)
          if (res?.code === 0) {
