                    ...
                    <div class="ml-auto flex shrink-0 gap-2">
                      ...
                      >
                        <el-button
                          link
                          size="small"
                          :disabled="CodexSetup.isStartingSessionInTerminal(group.workDir)"
...
<script lang="tsx" setup>
  ...
  import { CodexSetup, SessionItem } from './setup'
  ...
  const filteredGroups = computed(() => {
    const groups = CodexSetup.sessionGroups
    if (!search.value) {
      return groups
    }
  ...
  const deleteSelectedSessions = () => {
    ...
    ElMessageBox.confirm(I18nT('base.delAlertContent'), I18nT('base.delAlertTitle'), {
      ...
      .then(async () => {
        const deletedIds = await CodexSetup.deleteSessions(ids)
        const next = new Set(selectedSessionIds.value)
  ...
  const resumeLastSessionInTerminal = (workDir: string) => {
    CodexSetup.resumeLastSessionInTerminal(workDir)
  }
  ...
  const resumeSession = (sessionId: string, workDir: string) => {
    CodexSetup.resumeSession(sessionId, workDir)
  }
  ...
  const deleteSession = (row: SessionItem) => {
    ElMessageBox.confirm(I18nT('base.delAlertContent'), I18nT('base.delAlertTitle'), {
      ...
      .then(() => {
        CodexSetup.deleteSession(row.id)
      })
      .catch(() => {})
  }
