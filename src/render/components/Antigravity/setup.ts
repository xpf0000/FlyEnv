...
class Antigravity {
  ...
  deleteSessions(sessionIds: string[]): Promise<string[]> {
    ...
    return new Promise((resolve) => {
      IPC.send('app-fork:antigravity', 'deleteSessions', ids).then((key: string, res: any) => {
        IPC.off(key)
  ...
  startSessionInTerminal(workDir: string): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      IPC.send('app-fork:antigravity', 'runInTerminal', workDir).then((key: string, res: any) => {
        IPC.off(key)
  ...
  resumeLastSessionInTerminal(workDir: string): Promise<boolean> {
    ...
    this.openingSessionDirs.add(workDir)
    return new Promise((resolve) => {
      IPC.send('app-fork:antigravity', 'resumeLastSessionInTerminal', workDir).then(
        (key: string, res: any) => {
          IPC.off(key)
          this.openingSessionDirs.delete(workDir)
          if (res?.code === 0) {
