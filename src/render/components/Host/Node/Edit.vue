  ...
  >
    <div class="host-edit">
      ...
      <div class="main-wapper">
        ...
        <div class="plant-title">scripts</div>
        <div class="main">
          <el-select v-model="item.packageScript" class="w-full">
            <template v-for="(command, name) in scripts" :key="name">
              <el-option :label="`${name}: ${command}`" :value="name"></el-option>
            </template>
          </el-select>
        </div>
...
<script lang="ts" setup>
  ...
  watch(
    () => `${item.value.nodeDir}-${item.value.bin}`,
    () => {
      item.value.startCommand = `node.exe "${item.value.bin}"`
    }
  )
