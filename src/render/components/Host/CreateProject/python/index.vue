<template>
  <div class="h-full overflow-hidden flex flex-col">
    <el-autocomplete
      v-model="ProjectSetup.search.Python"
    ...
    <div class="flex-1 overflow-hidden">
      <el-scrollbar>
        <el-collapse v-model="ProjectSetup.collapse.Python">
...
<script lang="ts" setup>
  ...
  const list = computed(() => {
    ...
    if (ProjectSetup.search.Python) {
      const queryString = ProjectSetup.search.Python.toLowerCase()
  ...
  if (!ProjectSetup.collapse['Python']) {
    const keys = list.value.map((l) => l.key)
    ProjectSetup.collapse['Python'] = reactive([...keys])
  }
