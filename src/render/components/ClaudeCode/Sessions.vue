  ...
  <el-card class="version-manager">
    <template #header>
      <div class="card-header">
        ...
        <el-button
          link
          :disabled="ClaudeCodeSetup.sessionLoading"
          @click="ClaudeCodeSetup.refreshSessions()"
        >
          <yb-icon
            :svg="import('@/svg/icon_refresh.svg?raw')"
            class="w-[24px] h-[24px]"
            :class="{ 'fa-spin': ClaudeCodeSetup.sessionLoading }"
          ></yb-icon>
        </el-button>
      </div>
    </template>
    <div v-loading="ClaudeCodeSetup.sessionLoading" class="w-full h-full overflow-hidden">
