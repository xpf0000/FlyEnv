  ...
  <el-card class="version-manager">
    <template #header>
      <div class="card-header">
        <span>{{ I18nT('claudeCode.sessions') }}</span>
        <el-button
          link
          :disabled="ClaudeCodeSetup.loading"
          @click="ClaudeCodeSetup.refreshSessions()"
        >
          <yb-icon
    ...
    <div class="w-full h-full overflow-hidden">
      <div class="p-5 h-full overflow-hidden flex flex-col">
        <el-input
          v-model="search"
          :placeholder="I18nT('claudeCode.searchSession')"
          clearable
          class="mb-3"
        />
...
<script lang="tsx" setup>
  ...
  const filteredGroups = computed(() => {
    const groups = ClaudeCodeSetup.sessionGroups
    if (!search.value) {
      return groups
    }
  ...
  const resumeSession = (sessionId: string, workDir: string) => {
    ClaudeCodeSetup.resumeSession(sessionId, workDir)
  }
  ...
  const deleteSession = (row: SessionItem) => {
    ElMessageBox.confirm(I18nT('base.delAlertContent'), I18nT('base.delAlertTitle'), {
      ...
      .then(() => {
        ClaudeCodeSetup.deleteSession(row.id)
      })
      .catch(() => {})
  }
  ...
  const columns: Column<SessionItem>[] = [
    {
      key: 'id',
      title: I18nT('claudeCode.sessionId'),
      dataKey: 'id',
      width: 180,
      align: 'center',
      headerCellRenderer: () => (
        <span class="flex items-center justify-center">{I18nT('claudeCode.sessionId')}</span>
      ),
    ...
    {
      key: 'title',
      title: I18nT('claudeCode.sessionTitle'),
      dataKey: 'title',
      width: 200,
      headerCellRenderer: () => (
        <span class="flex items-center">{I18nT('claudeCode.sessionTitle')}</span>
      ),
      cellRenderer: ({ rowData: row }) => (
    ...
    {
      key: 'lastPrompt',
      title: I18nT('claudeCode.lastPrompt'),
      dataKey: 'lastPrompt',
      class: 'flex-1',
      headerClass: 'flex-1',
      width: 0,
      flexGrow: 1,
      headerCellRenderer: () => (
        <span class="flex items-center">{I18nT('claudeCode.lastPrompt')}</span>
      ),
      cellRenderer: ({ rowData: row }) => (
            ...
            default: () => (
              <ul class="host-list-menu">
                <li onClick={() => resumeSession(row.id, row.workDir)}>
                  ...
                  <span class="ml-3">{I18nT('claudeCode.resume')}</span>
                </li>
  ...
  onMounted(() => {
    ...
    ClaudeCodeSetup.refreshSessions()
  })
