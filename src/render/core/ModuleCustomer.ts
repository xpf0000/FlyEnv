import type { CustomerModuleServiceExecItem, CustomerModuleServiceItem } from '@/core/Module'
import { reactive } from 'vue'
import IPC from '@/util/IPC'
import { MessageError } from '@/util/Element'

const DefaultIcon = `<svg viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
>
  <path
...
class ModuleCustomerExecItem implements CustomerModuleServiceExecItem {
  command: string = ''
  comment: string = ''
...
class ModuleCustomer implements CustomerModuleServiceItem {
  ...
  constructor(item: any) {
    ...
    const arr: CustomerModuleServiceExecItem[] = item?.item ?? []
    for (const i of arr) {
      const execItem = new ModuleCustomerExecItem(i)
      list.push(execItem)
    }
