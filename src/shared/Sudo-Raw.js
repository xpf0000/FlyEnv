    ...
    if (typeof options.env !== 'object') {
      ...
      for (var key in options.env) {
        ...
        if (typeof key !== 'string' || typeof value !== 'string') {
          return end(new SudoPromptError('options.env environment variables must be strings.'))
        }
...
function Windows(instance, callback) {
  ...
  UUID(instance, function (error, uuid) {
    ...
    if (/"/.test(instance.path)) {
      ...
      return callback(new SudoPromptError('instance.path cannot contain double-quotes.'))
    }
