...
class Tangerine extends dns.promises.Resolver {
  ...
  static combineErrors(errors) {
    ...
    if (errors.length === 1) {
      ...
      if (errors[0].name !== undefined && errors.every((e) => e.name === errors[0].name))
        err.name = errors[0].name

      // if all errors had `code` and they were all the same then preserve it
      if (errors[0].code !== undefined && errors.every((e) => e.code === errors[0].code))
        err.code = errors[0].code

      // if all errors had `errno` and they were all the same then preserve it
      if (errors[0].errno !== undefined && errors.every((e) => e.errno === errors[0].errno))
  ...
  static ANY_TYPES = ['A', 'AAAA', 'CNAME', 'MX', 'NAPTR', 'NS', 'PTR', 'SOA', 'SRV', 'TXT']
  ...
  static RETRY_STATUS_CODES = new Set([408, 413, 429, 500, 502, 503, 504, 521, 522, 524])
  ...
  static getSysCall(rrtype) {
    return `query${rrtype.slice(0, 1).toUpperCase()}${rrtype.slice(1).toLowerCase()}`
  }
  ...
  constructor(options = {}, request = require('undici').request) {
    ...
    if (typeof request !== 'function')
      throw new Error('Request option must be a function (e.g. `undici.request` or `got`)')
    ...
    this.options = mergeOptions(
      {
        ...
        dnsOrder: semver.gte(process.version, 'v17.0.0') ? 'verbatim' : 'ipv4first',
    ...
    if (!['get', 'post'].includes(this.options.requestOptions.method.toLowerCase()))
    ...
    if (!(this.options.servers instanceof Set) || this.options.servers.size === 0)
      throw new Error('Servers must be an Array or Set with at least one server')
  ...
  setLocalAddress(ipv4, ipv6) {
    ...
    if (ipv4) {
      if (typeof ipv4 !== 'string') {
        const err = new TypeError('The "ipv4" argument must be of type string.')
    ...
    if (ipv6) {
      if (typeof ipv6 !== 'string') {
        const err = new TypeError('The "ipv6" argument must be of type string.')
        err.code = 'ERR_INVALID_ARG_TYPE'
        throw err
      }
  ...
  async lookup(name, options = {}) {
    ...
    if ((options?.hints & ~(dns.ADDRCONFIG | dns.ALL | dns.V4MAPPED)) !== 0) {
      const err = new TypeError(`The argument 'hints' is invalid. Received ${options.hints}`)
      err.code = 'ERR_INVALID_ARG_TYPE'
      throw err
    }
    ...
    if (answers.length > 0)
      answers =
        answers[0].length > 0 && (options.family === undefined || options.family === 0)
          ? answers[0]
          : answers.flat()
      ...
      switch (options.hints) {
        case dns.V4MAPPED: {
          if (options.family === 6 && !answers.some((answer) => isIPv6(answer)))
            answers = answers.map((answer) => ipaddr.parse(answer).toIPv4MappedAddress().toString())
          break
        }
    ...
    if (options.family === 4) answers = answers.filter((answer) => isIPv4(answer))
    else if (options.family === 6) answers = answers.filter((answer) => isIPv6(answer))
  ...
  async lookupService(address, port, abortController, purgeCache = false) {
    if (!address || !port) {
      const err = new TypeError('The "address" and "port" arguments must be specified.')
      err.code = 'ERR_MISSING_ARGS'
      throw err
    }

    if (!isIP(address)) {
      const err = new TypeError(`The argument 'address' is invalid. Received '${address}'`)
      err.code = 'ERR_INVALID_ARG_VALUE'
      throw err
    }

    if (!this.constructor.isValidPort(port)) {
      const err = new TypeError(`Port should be >= 0 and < 65536. Received ${port}.`)
    ...
    // reverse lookup
    try {
      const [hostname] = await this.reverse(address, abortController, purgeCache)
  ...
  async reverse(ip, abortController, purgeCache = false) {
    ...
    // perform resolvePTR
    try {
      const answers = await this.resolve(name, 'PTR', { purgeCache }, abortController)
      return answers
    } catch (err) {
      // remap syscall
      err.syscall = 'getHostByAddr'
  ...
  async #query(name, rrtype = 'A', ecsSubnet, abortController) {
    ...
    const pkt = dohdec.DNSoverHTTPS.makePacket({
      id: typeof this.options.id === 'function' ? await this.options.id() : this.options.id,
      rrtype,
    ...
    try {
        ...
        for (let i = 0; i < this.options.tries; i++) {
          try {
            ...
            if (response) {
              ...
              const message = http.STATUS_CODES[statusCode] || this.options.defaultHTTPErrorMessage
            ...
            // eslint-disable-next-line max-depth
            if (
              !(err.statusCode && this.constructor.RETRY_STATUS_CODES.has(err.statusCode)) &&
      ...
      if (errors.length > 0) this.options.logger.error(this.constructor.combineErrors(errors))
  ...
  #resolveByType(name, options = {}, parentAbortController) {
    return async (type) => {
      ...
      try {
        switch (type) {
          case 'A': {
            const result = await this.resolve4(name, { ...options, ttl: true }, abortController)
            return result.map((r) => ({ type, ...r }))
          }

          case 'AAAA': {
            const result = await this.resolve6(name, { ...options, ttl: true }, abortController)
            return result.map((r) => ({ type, ...r }))
          }

          case 'CNAME': {
            const result = await this.resolveCname(name, options, abortController)
          ...
          case 'NAPTR': {
            const result = await this.resolveNaptr(name, options, abortController)
            return result.map((value) => ({ type, value }))
          }
          ...
          case 'PTR': {
            const result = await this.resolvePtr(name, options, abortController)
            return result.map((value) => ({ type, value }))
          }

          case 'SOA': {
            const result = await this.resolveSoa(name, options, abortController)
            return { type, ...result }
          }

          case 'SRV': {
            const result = await this.resolveSrv(name, options, abortController)
            return result.map((value) => ({ type, value }))
          }

          case 'TXT': {
            const result = await this.resolveTxt(name, options, abortController)
  ...
  setDefaultResultOrder(dnsOrder) {
    if (dnsOrder !== 'ipv4first' && dnsOrder !== 'verbatim') {
      const err = new TypeError('The argument \'dnsOrder\' must be one of: \'verbatim\', \'ipv4first\'.')
      err.code = 'ERR_INVALID_ARG_VALUE'
      throw err
    }
  ...
  setServers(servers) {
    if (!Array.isArray(servers) || servers.length === 0) {
      const err = new TypeError('The "name" argument must be an instance of Array.')
      err.code = 'ERR_INVALID_ARG_TYPE'
    }
  ...
  spoofPacket(name, rrtype, answers = [], json = false, expires = 30000) {
    ...
    if (typeof rrtype !== 'string') {
      const err = new TypeError('The "rrtype" argument must be of type string.')
      err.code = 'ERR_INVALID_ARG_TYPE'
      throw err
    }
    ...
    const obj = {
      ...
      ttl: 300,
      expires: expires instanceof Date ? expires.getTime() : Date.now() + expires
    }
  ...
  async resolve(name, rrtype = 'A', options = {}, abortController) {
    ...
    const key = (ecsSubnet ? `${rrtype}:${ecsSubnet}:${name}` : `${rrtype}:${name}`).toLowerCase()
    ...
    switch (result.rcode) {
      case 'NOERROR': {
        ...
        if (result.flag_tc) {
          ...
          if (!Number.isFinite(ttl) || ttl < 1 || ttl > this.options.maxTTLSeconds)
    ...
    switch (rrtype) {
      ...
      case 'CERT': {
        ...
        return result.answers.map((answer) => {
          if (!Buffer.isBuffer(answer.data)) throw new Error('Buffer was not available')
          ...
          try {
            ...
            if (this.constructor.CTYPE_BY_VALUE[obj.certificate_type])
              obj.certificate_type = this.constructor.CTYPE_BY_VALUE[obj.certificate_type]
