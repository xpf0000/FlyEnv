...
import { appDebugLog, uuid, waitTime } from './utils'
...
async function macOpen(instance: Instance): Promise<{ stdout: string; stderr: string }> {
  ...
  return (await execChildProcessAsync('./' + path.basename(binary), options)) as any
}
...
async function windowsWriteExecuteScript(instance: Instance): Promise<void> {
  ...
  script.push(`set EXIT_CODE=!ERRORLEVEL!`)
  script.push(`(echo !EXIT_CODE!) > "${instance.pathStatus!}.tmp"`)
...
async function windows(instance: Instance): Promise<{ stdout: string; stderr: string }> {
  ...
  appDebugLog('[sudo][exec][instance]', `${JSON.stringify(instance)}`).catch()
  ...
  try {
    ...
    appDebugLog('[sudo][exec][error]', `${error}`).catch()
    try {
      await remove(instance.path!)
    } catch (removeError) {
      console.error('Error during cleanup:', removeError)
