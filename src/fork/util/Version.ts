 ...
 */
export const sdkmanJavaSearch = async (initPrefix: string) => {
  try {
    const command = `bash -c '${initPrefix} && sdk list java 2>/dev/null'`
    const res = await execPromiseWithEnv(command, {}, 30000)
    ...
    for (const line of lines) {
      ...
      items.push({
        name: identifier,
        version: version,
        installed: status.toLowerCase().includes('installed') || status.toLowerCase().includes('local'),
        flag: 'sdkman',
 ...
 */
export const sdkmanMavenSearch = async (initPrefix: string) => {
  try {
    const command = `bash -c '${initPrefix} && sdk list maven 2>/dev/null'`
    const res = await execPromiseWithEnv(command, {}, 30000)
    const stdout = res.stdout
    const lines = stdout.split('\n')
    const items: any[] = []
    ...
    for (const line of lines) {
      ...
      if (line.includes('===') || line.includes('---') || line.includes('local version') || !line.trim()) {
        continue
      }
      // Extract version strings from the line
      // Format: "     4.0.0-rc-5          3.9.1               3.5.4"
      // or:     " > * 3.9.14              3.8.9               3.5.3"
      const tokens = line.replace(/[>*]/g, ' ').trim().split(/\s+/).filter((t: string) => t.length > 0)
