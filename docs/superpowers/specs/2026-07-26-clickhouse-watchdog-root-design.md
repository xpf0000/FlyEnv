...
- If the registered/version PID is already absent, do not signal it. Still return that logical registered PID for main-process deregistration, then remove the version PID file. This is registry cleanup, not a process kill.
...
- `APP-Service-Stop-PID` remains the source of main-process deregistration. A replacement must delete the previous root registration before adding the new root registration.
...
- A stale PID is removed only from FlyEnv's PID file and in-memory registry.
...
2. A stale registered PID is returned for deregistration without appearing in the kill set.
