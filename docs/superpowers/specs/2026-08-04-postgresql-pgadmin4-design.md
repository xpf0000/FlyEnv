...
5. On first initialization only, run `setup.py setup-db` with `PGADMIN_SETUP_EMAIL` and `PGADMIN_SETUP_PASSWORD` in its short-lived child environment: pgAdmin 9.17's first SQLite migration uses those official environment variables to create the internal `Administrator` without prompting. A FlyEnv-owned, no-secret bootstrap script then uses pgAdmin's real `create_app` and models to confirm that administrator. After importing `servers.json`, the same no-secret bootstrap clears the connection password and normalizes `save_password` to `0`, because pgAdmin's importer otherwise leaves that column `NULL` for password-free records. A separate no-secret verifier confirms the exact FlyEnv user-owned connection, including the `Servers` group and `sslmode=prefer`. The administrator password never appears in argv, logs, config, or FlyEnv settings. FlyEnv writes `pgadmin4/initialized` only after that verifier exits successfully; the SQLite file or a CLI exit code alone is never an initialization signal.
...
- first-run credentials are consumed only by the short-lived `setup-db` environment and are not represented in persisted config or long-running service parameters;
...
The opt-in upstream check is `yarn test:postgresql-pgadmin4:integration`. It requires `PGADMIN4_INTEGRATION_PYTHON` to point to an environment with exactly `pgadmin4==9.17`. It creates and removes an isolated temporary pgAdmin root, writes the generated config, server registration, bootstrap, and verifier files, then executes real `setup-db`, bootstrap, `load-servers`, bootstrap normalization, verifier, and an independent model query. The query checks the Administrator, `Servers` group, loopback PostgreSQL record, `sslmode=prefer`, `save_password=0`, and blank password without putting the generated password in arguments or output.
