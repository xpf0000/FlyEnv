...
`syncServiceStatusFromMcp()` will skip any installed version whose `running` field is already `true`.
...
Only the renderer service-status synchronization function and its dedicated test change. Main-process PID tracking, IPC ordering, module lifecycle implementations, and startup-group state ownership remain unchanged.
