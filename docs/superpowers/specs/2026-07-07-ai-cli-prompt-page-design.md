...
- Why:
  `Start with environment control so the viewer sees FlyEnv MCP in action immediately.`
- Prompt:
...
- Why:
  `Use FlyEnv-provided context to bootstrap the database structure for the rest of the demo.`
- Prompt:
...
- Stage Type: `local shell step`
- Why:
  `Prepare visible initial data for the later browser CRUD flow.`
- Prompt:
...
- Why:
  `Generate a small framework-free app so the demo stays fast and easy to understand.`
- Note:
  `This is intentionally framework-free for recording speed and clarity.`
- Prompt:
...
- Why:
  `Turn the generated local files into an actual FlyEnv-managed site and launch the web stack.`
- Prompt:
...
- Why:
  `Create one clean on-screen checkpoint before switching to the browser.`
- Prompt:
...
- Why:
  `Prepare a clean script for the browser segment rather than another environment step.`
- Prompt:
