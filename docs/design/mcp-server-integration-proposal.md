...
| **S2: AI 自助排障** | 「我的站点 500 了」——AI 看不到 Nginx/PHP-FPM 日志，无法定位。 | `list_log_files(service)` tool 返回日志文件路径清单，AI 自行读取并分析栈 trace、给出修复建议。 |
...
| 重启 | `stopService` + `startService` | tool `restart_service`（内部先停止再启动） |
| 切「当前版本」 | **不是独立操作**，见 §3.2.1 | 并入 tool `start_service`（启动单实例服务某版本 = 切到该版本） |
| 在线版本 | `fetchAllOnlineVersion` | tool `list_online_versions`；`install_service` 内部复用 |
| 安装 | `installSoft` | tool `install_service`（**高风险，默认关闭，加入后需确认**） |
| 配置文件 | 各模块 `getConfigFiles(version)`（`Base` 默认返回 `[]`，有实际路径的模块覆写） | tool `list_config_files`（返回 name+path 清单，AI 自读） |
| 日志文件 | 各模块 `getLogFiles(version)`（`Base` 默认返回 `[]`，有实际路径的模块覆写） | tool `list_log_files`（返回 name+path 清单，AI 自读） |
...
1. 渲染端 `src/render/components/MCP/Module.ts`（`AppModuleItem`；参考 `ClaudeCode/Module.ts`，`moduleType: 'ai'`，`isService: true` —— MCP Server 本身是个可启停的本地服务，需要参与群组启停）。
2. 类型登记 `src/render/core/type.ts`：`AppModuleEnum.mcp`（命名注意与现有 `claudeCode` 等驼峰风格一致）。
3. MCP 的能力调用全部在 **main 进程** 的 `MCPTools.ts` 中通过 `ForkManager.send` 直接复用现有 fork 模块（`version`/`host`/`nginx`/...），因此**不需要**在 `src/fork/BaseManager.ts` 新增 `mcp` 分支，也**不需要** `src/fork/module/MCP/index.ts`。
...
| Resource URI | 内容 | 底层 |
|--------------|------|------|
| `flyenv://services` | 所有模块及安装/运行状态（包含各服务的已装版本列表） | `AppModuleEnum` + 各模块 status / `allInstalledVersions` |
| `flyenv://sites` | 本地站点列表（域名/根目录/PHP 版本/SSL） | `Host.hostList()` |

> 说明：`flyenv://services/{flag}/versions` 与 `flyenv://env` 未在当前实现中注册。各服务的版本信息已包含在 `flyenv://services` 返回的 `versions` 字段里；环境摘要暂未暴露为 resource。
...
| `create_site` / `update_site` | `AppHost` 字段 | 中-高 | `create/update` 默认需确认；`update_site` 可改 `phpVersion` 实现按站点切 PHP 版本 |
| `install_service` / `delete_site` | `flag`, `version` / `siteName` | 高 | 默认关闭；加入白名单后仍需确认 |
...
3. **工具白名单**：`MCPConfigManager` 的 `enabledTools`，用户可关闭任意高风险 tool；默认开启「只读（含 `list_online_versions`）+ 启停 + 站点创建/更新」，**安装服务 / 删除站点默认关闭**。
...
src/main/core/
  ...
  MCPAudit.ts                  # 新增：审计日志
  MCPBridgeManager.ts          # 新增：stdio bridge 运行时外部路径管理
  IPCHandler.ts                # 改：注册 mcp:start / mcp:stop / mcp:status / mcp:getConfig / mcp:setConfig / mcp:getBridgePath / mcp:getAuditLog
  # ConfigManager.ts 不改 —— MCP 配置独立，不进 ConfigOptions

# 注意：MCP 不新增 fork 模块，直接复用现有 fork 模块

src/fork/module/Base/index.ts  # 改：新增 `getConfigFiles` / `getLogFiles` 默认实现；各服务模块可覆写自己的配置/日志路径

static/mcp/
  flyenv-mcp-stdio.mjs         # 新增：stdio bridge 启动脚本（转发到运行中的 FlyEnv）

src/render/components/MCP/
  Module.ts                    # 新增：模块声明（moduleType:'ai'，isService: true，isTray: true）
  Index.vue                    # 新增：Tab 容器（服务 / 客户端配置 / 工具开关 / 审计）
  Service.vue                  # 新增：启停 + 端口/token + 远程开关
  ClientConfig.vue             # 新增：HTTP/stdio 配置片段 + 一键加到 AI CLI
  Tools.vue                    # 新增：工具白名单 + 审批策略开关
  Audit.vue                    # 新增：审计日志查看
  aside.vue                    # 新增：左侧导航 + 运行状态灯
  ASide.ts                     # 新增：Aside 状态与群组启停适配
  setup.ts                     # 新增：状态机与 IPC 封装

src/render/core/type.ts        # 改：AppModuleEnum.mcp
src/lang/{en,zh}/mcp.json      # 新增：文案
src/render/svg/mcp.svg         # 新增：图标
```
...
```jsonc
{
  "mcpServers": {
    "flyenv": {
      "command": "node",
      "args": ["<FlyEnv 外部目录>/mcp/flyenv-mcp-stdio.mjs"],
      "env": {
...
| 风险 | 说明 | 对策 |
|------|------|------|
| **AI 误操作破坏环境** | AI 可能调用 `delete_site` / `install_service` 造成损坏 | 风险分级 + 默认关闭高风险 tool + 高风险操作原生确认；配置文件由 AI 代理通过 `list_config_files` 拿到路径后自行读写，FlyEnv 不再提供 `write_config` tool |
...
- [ ] `MCPTools.ts`：接通 `ForkManager`，实现只读 resources（services / sites）+ 只读 tools（`service_status` / `list_log_files` / `list_config_files`）+ 启停三件套（start/stop/restart）。
...
- [ ] type.ts / **新增 `MCPConfigManager`（独立 mcp.json）** / i18n(en+zh) / 图标。
- [ ] 验收：在 Claude Code 里配置 FlyEnv MCP，能 `list services`、读 Nginx 日志、重启 Nginx。

### 二期（操作面补全 + stdio）
- [x] `list_config_files` / `list_log_files` / `list_online_versions`：配置/日志/线上版本**文件清单或版本列表**（name+path，AI 自读）。路径方法 `getConfigFiles`/`getLogFiles` 放在 **`Base` + 各模块覆写**（不再集中在一个 `MCPPaths`），新模块自带、单点维护；支持多文件（如 nginx/apache 的 error+access、mysql 的 error+slow）。早期 `read_log` 错误假设了不存在的 `getLogs()`，已废弃。
- [x] 切版本并入 `start_service`（单实例服务启动某版本即切到该版本，停其它+写 `current`；废弃独立的 `set_current_version`，FlyEnv 本无此独立概念）。
- [x] `create_site` / `update_site`（走 `Host`）；`set_site_php_version` 不再设独立 tool，按站点切 PHP 版本通过 `update_site(siteName, { phpVersion })` 完成。
- [x] stdio bridge 脚本，覆盖 Cursor / Cline / Windsurf。
- [x] 工具白名单 UI（Tools.vue）+ 写操作原生确认。

### 三期（安全与体验打磨）
- [x] `install_service` / `delete_site`（默认需确认）+ 审计日志 UI（Audit.vue）。
- [x] 输出脱敏、Origin 校验、远程访问开关（强提示）。
- [x] 托盘状态、主界面侧边栏开关。
- [ ] 多语言补全、官网 guide「用 Claude/Cursor 驱动 FlyEnv」。
...
4. **默认 tool 集**：默认开启「只读（含 `list_online_versions`） + 启停 + 站点创建/更新」，安装服务 / 删除站点默认关闭。是否符合预期？
  - 可以
...
**后端（main 进程）**
- `src/main/core/MCPConfigManager.ts`：独立 electron-store（`name:'mcp'` → `mcp.json`），`MCPConfigOptions`（enabled/transport/host/port/token/enabledTools/approval/allowRemote）；默认白名单为「只读（含 `list_online_versions`） + 启停 + `create_site` / `update_site`」，启停类与站点操作默认 `approval: confirm`；首次启动自动生成 token；`allowRemote` 默认 false。
- `src/main/core/MCPTools.ts`：tool/resource → `ForkManager.send(module, fn, ...)` 映射；新增 `listOnlineVersions`，删除 `writeConfig`。`serializeVersion`/`serializeSite` **默认返回 bin/path/凭据**（本地开发工具，执行任务所需）；可选 `maskSecrets`（默认 false）开启后才剔除凭据并对日志/配置掩码。
- `src/main/core/MCPServer.ts`：SDK 低层 `Server` + `StreamableHTTPServerTransport`（无状态模式），绑 `127.0.0.1`，Bearer token + 回环校验；`flag` 参数从 `AppModuleEnum` 动态生成；注册 tool（list_services / service_status / list_sites / list_log_files / list_config_files / list_online_versions / start/stop/restart_service / create_site / update_site / delete_site / install_service）+ 2 个 resource（services/sites），按白名单过滤。
...
- `src/main/core/IPCHandler.ts`：新增普通命令 `mcp:start` / `mcp:stop` / `mcp:status` / `mcp:getConfig` / `mcp:setConfig` / `mcp:getBridgePath` / `mcp:getAuditLog`，依赖接口新增 `mcpConfigManager` / `mcpServer` / `mcpBridgeManager`。

**渲染层** — `src/render/components/MCP/`
- `Module.ts`（`moduleType:'ai'`，`isService: true`，`isTray: true`，与 ClaudeCode/Codex/OpenCode 同分类，`asideIndex:106`）、`ASide.ts`、`aside.vue`、`Index.vue`（四 Tab：服务 / 客户端配置 / 工具开关 / 审计）、`Service.vue`（启停/端口/token/远程开关）、`ClientConfig.vue`（HTTP/stdio 配置片段 + 一键加到 AI CLI）、`Tools.vue`（白名单 + 审批策略开关）、`Audit.vue`（审计日志查看）、`setup.ts`（`reactiveBind` 状态机）。
...
- ✅ 默认开启「只读（含 `list_online_versions`） + 启停 + 站点创建/更新」，安装服务 / 删除站点默认关闭
...
| 3 | **切版本并入 `start_service`（已解决）** | 废弃独立 `set_current_version`——FlyEnv 中启动单实例服务某版本即切到该版本。PHP 按站点切版本通过 `update_site` 的 `phpVersion` 字段完成，不再设独立 tool。 | ✅ 已完成 |
| 4 | **安装 / 删除站点等高风险 tool** | 已实现（`install_service` / `delete_site`），默认不在白名单，加入后需确认 | ✅ 已完成 |
| 5 | **stdio bridge** | 已实现，bridge 脚本构建后复制到 `dirname(BaseDir)/mcp/` | ✅ 已完成 |
| 6 | **`MCPAudit.ts` 审计日志** | 已实现，UI 第四 Tab 可查看 | ✅ 已完成 |
...
> 状态：**已完成**。本期补全站点操作面、stdio bridge 与工具审批策略 UI。

### 13.1 与方案原设计的调整

| 原方案 | 实际实现 | 原因 |
|--------|---------|------|
| 独立 tool `set_site_php_version` | 并入 `update_site` | `update_site(siteName, { phpVersion })` 已能完成按站点切 PHP 版本，单独 tool 冗余 |
| `static/mcp/flyenv-mcp-stdio.mjs` 直接作为客户端配置中的 `command` | 构建时 bundle + 运行时复制到外部路径 | 生产包 asar 内文件无法被外部 `node` 进程执行；bundle 后无外部依赖，再复制到 `dirname(global.Server.BaseDir)/mcp/` |

### 13.2 新增 / 改动文件

**后端（main 进程）**
- `src/main/core/MCPTools.ts`：新增 `createSite` / `updateSite` / `findSiteByName`，废弃 `setSitePhpVersion`。
- `src/main/core/MCPServer.ts`：注册 `create_site` / `update_site` tool；高风险 tool 调用前按 `approval` 策略触发原生确认框。
- `src/main/core/MCPBridgeManager.ts`：新建。负责把 bundle 后的 bridge 脚本从 asar 内复制到 `dirname(global.Server.BaseDir)/mcp/flyenv-mcp-stdio.mjs`，并提供 `getBridgePath()`。
- `src/main/core/IPCHandler.ts`：新增 `mcp:getBridgePath` 命令。
- `src/main/Application.ts`：实例化 `MCPBridgeManager` 并注入 `IPCHandler`。
- `src/main/core/MCPConfigManager.ts`：默认白名单与 approval 加入 `create_site` / `update_site`。

**构建**
- `configs/plugs.build.ts`：static 复制后，用 esbuild 把 `static/mcp/flyenv-mcp-stdio.mjs` bundle 成单文件（含 SDK，无外部 `node_modules` 依赖），输出到 `dist/electron/static/mcp/flyenv-mcp-stdio.mjs`。

**渲染层**
- `src/render/components/MCP/setup.ts`：新增 `bridgePath`，初始化时通过 `mcp:getBridgePath` 获取外部可执行路径；`stdioConfigSnippet` 改用该路径。
- `src/render/components/MCP/ClientConfig.vue`：HTTP / stdio 两个 Tab，分别生成配置片段。
- `src/render/components/MCP/Tools.vue`：新增工具审批策略（auto / confirm）UI。
- `src/lang/{en,zh}/mcp.json`：补充 httpConfig / stdioConfig / approval 等文案。

### 13.3 验证

- `npx vue-tsc --noEmit`：改动文件 0 新增错误。
- `npx eslint`：改动文件无 lint 错误。
- `npx esbuild` 主进程打包成功。
- 手动 bundle bridge 脚本验证：输出单文件仅引用 `node:process`，`node --check` 通过。

---

## 14. 实施记录（三期）

> 状态：**核心安全功能已完成**。本期补齐高风险工具、审计日志、Origin 校验与远程访问强提示。

### 14.1 已交付功能

| 功能 | 实现 | 说明 |
|------|------|------|
| `delete_site` | `MCPTools.deleteSite` → `Host.handleHost(site, 'del')` | 默认不在白名单；加入后需确认 |
| `list_online_versions` | `MCPTools.listOnlineVersions` | 查询模块线上可用版本，供 `install_service` 前选择版本 |
| `install_service` | `MCPTools.installService` | 查在线版本列表 → 匹配版本 → 调用模块 `installSoft` |
| 审计日志 | `src/main/core/MCPAudit.ts` + `Audit.vue` | 所有 tool 调用（含成败）追加到 `BaseDir/mcp/audit.log`；UI 第四 Tab 查看 |
| Origin 校验 | `MCPServer.checkAuth` | 带 `Origin` 头时必须匹配 `http(s)://127.0.0.1:<port>` 或 `localhost:<port>`；远程模式额外允许配置的 host |
| 远程访问强提示 | `Service.vue` | 勾选「允许远程访问」时弹 `ElMessageBox.confirm` 二次确认 |
| 侧边栏开关 | `aside.vue` + `ASide.ts` | 主界面 Aside 增加 `el-switch`，可一键启停 MCP Server；注册到 `AppServiceModule` 以支持托盘命令回调 |
| 托盘状态指示 | `Module.ts` (`isTray: true`) | MCP Server 出现在托盘菜单（现代/经典两种风格），显示运行/停止状态并支持点击切换 |

### 14.2 默认安全策略

- `delete_site` / `install_service` **默认不在白名单**，用户需在 Tools Tab 手动开启。
- 上述工具即使被加入白名单，默认 `approval: confirm`，执行前弹出原生确认框。
- `maskSecrets` 已在 `MCPTools` 中实现：结构化输出默认返回 bin/path/凭据；开启后剔除 `rootPassword` 与 SSL 私钥。

### 14.3 仍待后续

- 其余 20+ 语言 `mcp.json` 补全（当前仅 en/zh）。
- 官网 / 文档 guide「用 Claude/Cursor 驱动 FlyEnv」。
