...
| `list_sites` / `create_site` / `update_site` | `AppHost` 字段 | 中-高 | `create/update` 默认需确认；`update_site` 可改 `phpVersion` 实现按站点切 PHP 版本 |
...
### 三期（安全与体验打磨）
- [x] `write_config` / `install_service` / `delete_site`（默认需确认）+ 审计日志 UI（Audit.vue）。
...
| `write_config` | `MCPTools.writeConfig` | 支持 `name`（从 `list_config_files` 选）或绝对 `path`；写入前自动 `.bak` 备份 |
...
- `delete_site` / `write_config` / `install_service` **默认不在白名单**，用户需在 Tools Tab 手动开启。
