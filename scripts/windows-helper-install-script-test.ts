...
assert.match(source, /\$hresult -band 0xffff\) -eq 33/)
assert.match(source, /function Test-SecureHelperKey/)
assert.match(source, /Test-SecureHelperKey -Path \$keyPath/)
assert.match(source, /Length -ne 32/)
assert.match(source, /Global\\FlyEnv\.Helper\.Install\.\$instanceId/)
assert.match(source, /WaitOne\(\[TimeSpan\]::FromSeconds\(30\)\)/)
assert.match(source, /catch \[System\.Threading\.AbandonedMutexException\]/)
assert.match(source, /SetAccessRuleProtection\(\$true, \$false\)/)
assert.match(source, /MutexAccessRule\(\$trustedSid, 'FullControl', 'Allow'\)\)/)
assert.match(source, /Another FlyEnv helper installation is active for SID/)
assert.match(source, /try \{ \$sidInstallMutex\.ReleaseMutex\(\) \} catch \{\}/)
assert.match(source, /RestartInterval = 'PT1M'/)
assert.match(source, /RestartCount = 3/)
assert.match(source, /StartWhenAvailable = \$true/)
assert.match(source, /ExecutionTimeLimit = 'PT0S'/)
assert.match(source, /AllowDemandStart = \$true/)
assert.match(source, /MultipleInstances = 2/)
assert.match(source, /DisallowStartIfOnBatteries = \$false/)
assert.match(source, /StopIfGoingOnBatteries = \$false/)
assert.match(
  source,
...
const stageOrder = (marker: string) => source.indexOf(marker)
assert.ok(
  stageOrder("$stage = 'stage-replacements'") < stageOrder("$stage = 'stop-current-instance'")
)
assert.ok(
  stageOrder("$stage = 'stop-current-instance'") < stageOrder("$stage = 'publish-replacements'")
)
assert.ok(stageOrder("$stage = 'publish-replacements'") < stageOrder("$stage = 'register-task'"))
assert.ok(stageOrder("$stage = 'register-task'") < stageOrder("$stage = 'start-task'"))
